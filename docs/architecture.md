...
- 후보 풀은 `chunk_id` 기준으로 중복 제거 후 최종 점수 내림차순으로 정렬합니다.
- 정렬된 후보 중 상위 N개를 리랭커에 전달합니다.

### 5.2 리랭킹
- CPU 기반 크로스 인코더 리랭커를 사용해 리랭킹 합니다.
- 리랭킹 단계는 필수로 적용하며, 최종 반환 Top-N은 리랭킹 점수 기준으로 결정합니다.
- 운영 환경의 자원 제약을 고려해 기본 실행 장치는 CPU로 둡니다.
...
- abstain 판단은 규칙 기반(근거 개수/유효성) 1차 게이트를 기본으로 하고,
  필요 시 LLM 보조 신호(`needs_abstain`)를 2차 게이트로 반영할 수 있습니다.
...
- 오프라인을 전제로, 아래 항목은 설정 파일/환경 변수로 고정 가능해야 합니다.
  - 모델 경로, 인덱스 저장 경로, 캐시 경로(HF_HOME 등), Top-K, 리랭킹 후보 수, 최대 컨텍스트 길이
  - llama.cpp GPU 오프로딩 파라미터(`n_gpu_layers`, `main_gpu`)를 통해 LLM 가중치를 VRAM에 우선 배치
